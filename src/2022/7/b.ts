...
const available = 70000000 - dirs["/"].size;
const deleteable = 30000000 - available;

let miniumum = 70000000;

function getScore(dirs: { [x: string]: any }) {
  for (const item in dirs) {
    if (item === "size" && dirs[item] >= deleteable) {
      console.log(dirs[item]);
      miniumum = Math.min(dirs[item], miniumum);
      continue;
    }
